...
class SoftActorCritic(nn.Module):
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            action = torch.stack([action_distribution.sample() for _ in range(self.num_actor_samples)], dim=0)
            assert action.shape == (
            ...
            q_values = torch.stack([self.critic(obs, action[i]) for i in range(self.num_actor_samples)], dim=1)
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        # q_values has shape (num_critic_networks, batch_size), take mean over both dimensions
    ...
    def update(
        ...
        for n in range(self.num_critic_updates):
